> Portfolio snapshot note: the upstream text references a NOTICE file that is not present in this local snapshot. Its references are shown as plain text; no replacement notice has been invented.
...
`NOTICE` file, the project's list of copyright holders and authors.
...
copyright holders in the `NOTICE` file. As author of a corporate
